<template>
  <el-card class="box-card" shadow="always">
    ...
    >
      ...
      <el-table-column prop="roomId" label="编号" width="50" />
      <el-table-column sortable prop="roomNumber" label="房间号码">
      ...
      <el-table-column prop="roomPrice" sortable label="价格" />
      <el-table-column sortable prop="roomDiscount" label="折扣">
        <template slot-scope="scope">
          <el-tag type="primary">{{ scope.row.roomDiscount + "%" }}</el-tag>
        </template>
      </el-table-column>

      <el-table-column prop="roomStatus" sortable label="状态">
        <template slot-scope="scope">
          <!--<el-tag :type="showStatus(scope.row.roomStatus)">{{ scope.row.roomStatus }}</el-tag>-->
          <el-button :type="showStatus(scope.row.roomStatus)" size="mini"
            >{{ scope.row.roomStatus | formatStatus }}
          </el-button>
        </template>
      </el-table-column>
      <el-table-column sortable prop="createTime" label="预订时间">
        <template slot-scope="scope">
      ...
      <el-table-column sortable prop="updateTime" label="入住时间">
        <template slot-scope="scope">
          <i class="el-icon-time" />
          <span style="margin-left: 10px">{{
      ...
      <el-table-column fixed="right" label="操作">
        <template slot-scope="scope">
          <el-button
            ...
            v-if="user.deptName == '客房部' || user.deptName == '前台'"
            class="primary"
            >编辑
          </el-button>
          <el-popover v-model="scope.row.visible2" placement="top" width="160">
            ...
            <div style="text-align: right; margin: 0">
              <el-button size="mini" round @click="scope.row.visible2 = false"
                >取消</el-button
              >
              <el-button
                type="danger"
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      getAllRoom(this.currentPage, this.pageSize, this.search).then(
        response => {
